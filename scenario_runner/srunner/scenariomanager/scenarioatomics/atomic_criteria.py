...
class CollisionTest(Criterion):
    ...
    def _count_collisions(weak_self, event):     # pylint: disable=too-many-return-statements
        ...
        """collision_event.set_message( # NOTE: modified
        ...
        actor_transform = CarlaDataProvider.get_transform(self.actor)
        collision_event.set_message([
            round(actor_transform.location.x, 3),
            round(actor_transform.location.y, 3),
            round(actor_transform.location.z, 3),
            round(actor_transform.rotation.pitch, 3),
            round(actor_transform.rotation.yaw, 3),
            round(actor_transform.rotation.roll, 3)
            ])
...
class ActorSpeedAboveThresholdTest(Criterion):
    ...
    def _set_event_message(self, event, location):
        ...
        """event.set_message('Agent got blocked at (x={}, y={}, z={})'.format(round(location.x, 3), # NOTE: modified
                                                                           round(location.y, 3),
                                                                           round(location.z, 3)))"""
...
class OnSidewalkTest(Criterion):
    ...
    def _set_event_message(self, event, location, distance):
        ...
        """event.set_message( # NOTE: modified
...
class OutsideRouteLanesTest(Criterion):
    ...
    def terminate(self, new_status):
        ...
        if self._wrong_distance > 0:
            ...
            """outside_lane.set_message( # NOTE: modified
    ...
    def update(self):
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            ...
            if off_route:
                ...
                """route_deviation_event.set_message( # NOTE: modified
            ...
            for wp in waypoints:
                ...
                if tail_wp.road_id == wp.road_id and tail_wp.lane_id == wp.lane_id and dot_ve_wp > 0:
                    ...
                    if self.is_vehicle_crossing_line((tail_close_pt, tail_far_pt), (lft_lane_wp, rgt_lane_wp)):
                        ...
                        """red_light_event.set_message( # NOTE: modified
        ...
        if not self._target_stop_sign:
            ...
            if not self.is_actor_affected_by_stop(self._actor, self._target_stop_sign):
                ...
                if not self._stop_completed and self._affected_by_stop:
                    ...
                    """running_stop_event.set_message( # NOTE: modified
