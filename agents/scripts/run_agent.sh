...
if ${imitation_learning}; then # use offset agent (imitation learning model)
    ...
    if ${evaluate}; then # evaluate
        python3 ${BASE_CODE_PATH}/agents/_scenario_runner/scenario_runner.py --agent ${TEAM_AGENT} --route ${ROUTES} ${SCENARIOS} 0 --repetitions ${repetitions} --evaluate --imitation_learning
    else # train
        python3 ${BASE_CODE_PATH}/agents/_scenario_runner/scenario_runner.py --agent ${TEAM_AGENT} --route ${ROUTES} ${SCENARIOS} 0 --repetitions ${repetitions} --imitation_learning
    fi
