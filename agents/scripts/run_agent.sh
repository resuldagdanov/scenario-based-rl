...
if ${imitation_learning}; then # use offset agent (imitation learning model)
    ...
    if ${evaluate}; then # evaluate
        python3 ${BASE_CODE_PATH}/scenario_runner/scenario_runner.py --agent ${TEAM_AGENT} --route ${ROUTES} ${SCENARIOS} --repetitions ${repetitions} --evaluate --imitation_learning
    else # train
        python3 ${BASE_CODE_PATH}/scenario_runner/scenario_runner.py --agent ${TEAM_AGENT} --route ${ROUTES} ${SCENARIOS} --repetitions ${repetitions} --imitation_learning
    fi
    ...
    if ${evaluate}; then # evaluate
        python3 ${BASE_CODE_PATH}/scenario_runner/scenario_runner.py --agent ${TEAM_AGENT} --route ${ROUTES} ${SCENARIOS} --repetitions ${repetitions} --evaluate
    else # train
        python3 ${BASE_CODE_PATH}/scenario_runner/scenario_runner.py --agent ${TEAM_AGENT} --route ${ROUTES} ${SCENARIOS} --repetitions ${repetitions}
    fi
fi