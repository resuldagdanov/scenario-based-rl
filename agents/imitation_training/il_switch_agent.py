...
class SwitchAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file, route_id, rl_model=None):
        ...
        model_folder = "Jan_23_2022-22_07_43/"
    ...
    def run_step(self, input_data, timestamp):
        ...
        dnn_agent_brake, dnn_switch_decision = self.agent.inference(front_images=front_cv_image, waypoint_input=fused_waypoint_input, speed_sequence=np.array(self.speed_sequence))
        ...
        if float(dnn_switch_decision) > 0.5:
            switch = float(1)
        else:
            switch = float(0)
        ...
        print("[Action]:", throttle, steer, brake, " [Autopilot Brake]:", autopilot_brake, " [Reward]:", reward, " [Done]:", done, " [Switch?]:", switch)
