...
from networks.switch_network_three_class import SwitchNetwork
...
class SwitchAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file, route_id, rl_model=None):
        ...
        # brake classifier il model
        il_model_folder = "Jan_31_2022-22_11_05/"
        il_model_name = "epoch_49.pth"
        
        # traffic light rl model
        rl1_model_folder = "Jan_31_2022-12_02_19/"
        rl1_model_episode_number = 400

        # stuck vehicle rl model
        rl2_model_folder = "Jan_31_2022-12_02_19/"
        rl2_model_episode_number = 400

        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        print("device: ", self.device)

        self.brake_agent = SwitchNetwork()
        self.traffic_light_agent = DQNNetwork()
        self.stuck_vehicle_agent = DQNNetwork()

        self.brake_agent.to(self.device)
        self.traffic_light_agent.to(self.device)
        self.stuck_vehicle_agent.to(self.device)

        il_model_path = os.path.join(os.path.join(os.environ.get('BASE_CODE_PATH'), "checkpoint/models/" + il_model_folder), il_model_name)
        rl1_model_path = os.path.join(os.path.join(os.environ.get('BASE_CODE_PATH'), "checkpoint/models/" + rl1_model_folder), "dqn" + "-ep_" + str(rl1_model_episode_number))
        rl2_model_path = os.path.join(os.path.join(os.environ.get('BASE_CODE_PATH'), "checkpoint/models/" + rl2_model_folder), "dqn" + "-ep_" + str(rl2_model_episode_number))

        self.brake_agent.load_state_dict(torch.load(il_model_path))
        self.traffic_light_agent.load_state_dict(torch.load(rl1_model_path))
        self.stuck_vehicle_agent.load_state_dict(torch.load(rl2_model_path))

        self.brake_agent.eval()
        self.traffic_light_agent.eval()
        self.stuck_vehicle_agent.eval()
    ...
    def privileged_sensors(self):
        ...
        self.collision_sensor.listen(lambda event: SwitchAgent._on_collision(weakref.ref(self), event))
    ...
    def run_step(self, input_data, timestamp):
        ...
        fused_waypoint_input = np.zeros(2, dtype=np.float32)
        fused_waypoint_input[0] = near_node[0] - gps[0]
        fused_waypoint_input[1] = near_node[1] - gps[1]
        ...
        dnn_agent_brake, dnn_switch_decision = self.brake_agent.inference(front_images=front_cv_image, waypoint_input=fused_waypoint_input, speed_sequence=np.array(self.speed_sequence))

        # 0: IL Brake Agent     1: Traffic Light RL Agent       2: Stuck Vehicle RL Agent
        switch_policy_idx = int(dnn_switch_decision)

        if switch_policy_idx == 0:
            dnn_agent_brake = dnn_agent_brake

            # apply braking or not
            ...
            if switch_policy_idx == 1:
                dnn_agent_action = self.traffic_light_agent.inference(front_images=front_cv_image)
            elif switch_policy_idx == 2:
                dnn_agent_action = self.stuck_vehicle_agent.inference(front_images=front_cv_image)
            else:
                print("Agent IDs are Different !!!")
                sys.exit()

            throttle, steer, brake, angle = self.calculate_high_level_action(dnn_agent_action, ego_theta, gps, near_node, far_node, data)
        ...
        autopilot_brake = any(x is not None for x in [is_light, is_walker, is_vehicle])
        ...
        print("[Action]:", throttle, steer, brake, " [Autopilot Brake]:", autopilot_brake, " [Reward]:", reward, " [Done]:", done, " [Switch Policy ID]:", switch_policy_idx)
    ...
    def calculate_high_level_action(self, high_level_action, compass, gps, near_node, far_node, data):
        ...
        steer, throttle, brake, target_speed, angle = self.get_control(new_near_node, far_node, data, brake)

        return throttle, steer, brake, angle

    def destroy(self):
        del self.brake_agent
        del self.traffic_light_agent
        del self.stuck_vehicle_agent
