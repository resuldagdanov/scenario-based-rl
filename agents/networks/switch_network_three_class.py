...
class SwitchNetwork(nn.Module):
    def __init__(self):
        ...
        # switch classifier between IL and RL Agents branch
    ...
    def inference(self, front_images, waypoint_input, speed_sequence):
        ...
        # inference
        with torch.no_grad():
            brake_torch, switch_class_torch = self.forward(front_images=front_images_torch, waypoint_input=waypoint_input_torch, speed_sequence=speed_sequence_torch)
        
        # torch control and switch decision to CPU numpy array
        brake = brake_torch.squeeze(0).cpu().detach().numpy()

        # find best class to switch to out of 3 classes
        switch_class_idx = torch.argmax(nn.functional.softmax(switch_class_torch))

        return brake, switch_class_idx